...
function ChoiceSight() {
    ...
    useEffect(() => {
        ...
        axios.get("http://localhost:8000/recommendations")
            .then((response) => {  
                // const {content, totalPages} = response.data.modelsight_data; // FastAPI 서버의 응답 데이터
                const data = response.data.modelsight_data
                // console.log(response.data.modelsight_data)
                setSightList(data)
                // setSights(content);
                ...
                {sightList.map((sight,index) => {
                        return (
                            <div key={index} style={{display:'flex', alignItems:'center'}}>
                                <div>
                                    <h5><b>Name: {sight[4]}</b></h5>
                                    <p>Type: {sight[5]}</p>
                                    <p>Address: {sight[6] == '없음' ? sight[7] : sight[6]}</p>
                                    <p>⭐ {sight[8]}</p>
                                </div>
                        ...
                        return (
                            <div key={sight.sight_id} style={{display:'flex', alignItems:'center'}}>
                                <div>
                                    ...
                                    <p>Type: {sight.type}</p>
                                    <p>Address: {sight.address1 == '없음' ? sight.address2 : sight.address1}</p>
                                    <p>⭐: {sight.rating}</p>
                                </div>
                                <button style={{borderRadius:'5px', borderColor:'lightgray', marginLeft:'10px'}} className='btn btn-outline-secondary' onClick={() => handleSightSelect(sight)}>+</button>
                            </div>
